...
class ResUsers(models.Model):
    ...
    #     write function is overide by silverdaletech becasue issue in creating member in sales team , and need to create ticket for third party
    #         if they resolve this issue then remove this code and use updated code
    def write(self, vals):
        res = super(ResUsers, self).write(vals)
        for rec in self:
            if rec.has_group('amazon_ept.group_amazon_user_ept') or rec.has_group( \
                    'amazon_ept.group_amazon_manager_ept'):
                company_ids = rec.company_ids.ids
                rec.set_user_fba_fbm_group(company_ids)
        return res
