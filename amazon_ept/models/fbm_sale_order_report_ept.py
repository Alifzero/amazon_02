...
class FbmSaleOrderReportEpt(models.Model):
    ...
    def prepare_order_dict(self, order_dict, amz_ref, instance_id, row):
        ...
        fbm_order_dict = {
            ...
            'item-promotion-discount': row.get('item-promotion-discount', 0.0),
            'ship-promotion-discount': row.get('ship-promotion-discount', 0.0),
    ...
    def create_fbm_unshipped_chargable_order_line_ept(self, instance, order, order_detail):
        ...
        if float(order_detail.get( \
                'shipping-price', 0.0)) > 0 and instance.seller_id.shipment_charge_product_id:
        ...
        if float(order_detail.get(
            ...
            item_discount = float(
                order_detail.get('item-promotion-discount', 0.0)) * (-1)
        ...
        if float(order_detail.get( \
            ...
            ship_discount = float( \
                order_detail.get('ship-promotion-discount', 0.0)) * (-1)
