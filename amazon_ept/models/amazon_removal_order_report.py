    ...
    def amz_removal_canceled_qty_ept(self, row, picking_vals, quantity, job):
        ...
        if quantity:
            existing_move = self.amz_get_stock_move_from_picking_ept( \
                picking_vals.get('product_id', False), picking_vals.get('canceled_pickings', False), \
                picking_vals.get('source_location_id', False), picking_vals.get('location_dest_id', False))
            if existing_move:
