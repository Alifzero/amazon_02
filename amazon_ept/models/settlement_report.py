...
class SettlementReportEpt(models.Model):
    ...
    def reconcile_reimbursement_lines(self, seller, bank_statement, statement_lines, \
        ...
        for line in statement_lines:
            ...
            reimbursement_invoice = reimbursement_invoice_ids.filtered( \
                    lambda l: l.state in ['draft', 'posted'] and not l.payment_state in ['paid', 'in_payment'] and l.move_type == invoice_type)
    ...
    def process_settlement_orders(self, bank_statement, settlement_id, orders_list):
        ...
        bank_line_vals = [{'name': "%s/%s" % (order_key[5], order_key[0]), 'payment_ref': "%s/%s/%s" % (settlement_id, order_key[5], order_key[0]), 'partner_id':
    ...
    def check_or_create_invoice_if_not_exist(self, amz_order):
        ...
        for order in amz_order:
            ...
            order_credit_note = order.invoice_ids.filtered(lambda l: l.move_type == 'out_refund' and l.state not in ('cancel'))
            if order.state == 'sale' and order.invoice_status == 'to invoice' and not order_credit_note:
    ...
    def create_refund_invoices(self, refund_list_item_price, bank_statement):
        ...
        for order_key, product_amount in refund_list_item_price.items():
            ...
            if not refund_exist:
                ...
                refund_invoices = [refund.get('res_id', False)] if refund.get('res_id', False) else refund.get('domain', False)[0].__getitem__(2)
