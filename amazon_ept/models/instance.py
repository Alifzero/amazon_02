...
class AmazonInstanceEpt(models.Model):
    ...
    def get_compare_data(record, fulfillment_by_where_clause):
        ...
        def get_compared_week_data(record, fulfillment_by_where_clause):
            ...
            if current_week_data:
                current_total = current_week_data.get('current_week', 0.0) if current_week_data.get('current_week', 0.0) else 0
            # Previous week data
            ...
            if previous_week_data:
                previous_total = previous_week_data.get('previous_week', 0.0) if previous_week_data.get(
                    'previous_week', 0.0) else 0
        ...
        def get_compared_month_data(record, fulfillment_by_where_clause):
            ...
            current_total = current_data.get('current_month', 0.0) if current_data and current_data.get('current_month', 0.0) else 0
            # Previous week data
            ...
            previous_total = previous_data.get('previous_month', 0.0) if previous_data and previous_data.get('previous_month', 0.0) else 0
            return current_total, previous_total
        ...
        def get_compared_year_data(record, fulfillment_by_where_clause):
            ...
            record._cr.execute("""select sum(amount_untaxed) as current_year from sale_order
                                ...
                                and amz_instance_id={0} and state in ('sale','done') {1}""" .format(record.id,
                                                                                               fulfillment_by_where_clause))
            current_data = record._cr.dictfetchone()
            current_total = current_data.get('current_year', 0.0) if current_data and current_data.get('current_year', 0.0) else 0
            # Previous week data
            ...
            previous_data = record._cr.dictfetchone()
            if previous_data:
                previous_total = previous_data.get('previous_year', 0.0) if previous_data.get('previous_year', 0.0) else 0
            return current_total, previous_total
    ...
    def get_graph_data(record, fulfillment_by_where_clause):
        ...
        values = [{"x": ("{}".format(data.get(list(data.keys())[0], []))), "y": data.get('sum', 0.0) or 0.0} for data in result]
        return values
