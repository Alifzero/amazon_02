                ...
                if fulfillment_id not in fulfillment_warehouse:
                    ...
                    if not fn_warehouse:
                        ...
                        common_log_line_obj.amazon_create_order_log_line(
                            message, model_id, self.id, row.get('amazon-order-id', False), False, 'FBA', log_rec)
                        continue
            ...
            for shipment_id, shipment_lines in shipment_dict.items():
                ...
                for ship_line in shipment_lines:
                    if ship_line.get('tracking-number', False) and ship_line.get('tracking-number', False) not in track_list:
                        ...
                        track_num = ship_line.get('tracking-number', False) if not track_num else track_num +','+ship_line.get('tracking-number', False)
    ...
    def prepare_outbound_shipment_dict(lines):
        ...
        for line in lines:
            if line.get('shipment-id', False) not in shipment_dict:
                ...
                shipment_dict.update({line.get('shipment-id', False): shipment_dict.get(line.get('shipment-id', False)) + [line]})
        ...
        for data in order_line_data:
            order_line = order_lines.filtered(
                lambda x: x.amazon_order_item_id == data.get(
                    "amazon-order-item-id", False) and x.product_uom_qty == float(\
                    data.get("quantity-shipped", 0.0)) and x.order_id.warehouse_id.id == data.get(\
                    "warehouse", False))
