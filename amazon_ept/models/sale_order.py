...
class SaleOrder(models.Model):
    ...
    def prepare_amazon_sale_order_vals(instance, partner_dict, order):
        ...
        if order.get('PurchaseDate', {}).get('value', False):
            date_order = parser.parse(
                order.get('PurchaseDate', {}).get('value', False)).astimezone(utc).strftime( \
                '%Y-%m-%d %H:%M:%S')
        else:
    ...
    def check_already_status_updated_in_amazon(self, seller, marketplaceids, instances, next_token={}):
        ...
        next_token_wrapper = list_of_wrapper[-1].get('NextToken', {}).get('value', {}) if list_of_wrapper and isinstance(list_of_wrapper, list) else {}
    ...
    def amz_update_tracking_number(self, seller):
        ...
        while next_token or flag:
            amazon_orders, next_token = self.check_already_status_updated_in_amazon(seller, marketplaceids, seller.instance_ids, next_token)
            flag = False
